...
xmalloc(size_t alloc_sz)
{
  ...
  size_t padded_sz = alloc_sz ;

  if(alloc_sz>0x10000){
#ifdef POLYBENCH_ENABLE_INTARRAY_PAD
    //polybench_inter_array_padding_sz = rand()%64;
    //polybench_inter_array_padding_sz = rand()%2048;
    //polybench_inter_array_padding_sz += 9;
    //printf("inter-array padding = %lld\n", polybench_inter_array_padding_sz);
#else
    polybench_inter_array_padding_sz = 0;
#endif
    padded_sz = alloc_sz + ((64*polybench_inter_array_padding_sz));
    //printf("malloc   padded_sz, alloc_sz = %x %x\n",padded_sz, alloc_sz);
...
void* polybench_alloc_data(unsigned long long int n, int elt_size)
{
  ...
  if(seed==-1){
    //seed=time(NULL);
    seed=POLYBENCH_INTER_ARRAY_PADDING_FACTOR;
    srand(seed);
    //printf("seed, intra =  %lld  %lld\n",seed, POLYBENCH_PADDING_FACTOR);
...
void* polybench_alloc_data2(unsigned long long int n,unsigned long long int m, int elt_size)
{
  ...
  //  int r=rand()%32;
    int r=0;
  printf("intra-array padding = %lld\n",r);

  /// FIXME: detect overflow!
  size_t val = n*(m+r)* elt_size;
  void* ret = xmalloc (val);
